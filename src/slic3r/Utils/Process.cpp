...
// For starting another ModixSlicer instance on OSX.
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance, bool delete_after_load)
{
	...
	path += "\\";
	path += (instance_type == NewSlicerInstanceType::Slicer) ? "modix-slicer.exe" : "modix-gcodeviewer.exe";
	...
	// Don't call with wxEXEC_HIDE_CONSOLE, ModixSlicer in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#if defined(__APPLE__)
	{
		...
		// ((instance_type == NewSlicerInstanceType::Slicer) ? "ModixSlicer" : "PrusaGCodeViewer");
		// Just run ModixSlicer and give it a --gcodeviewer parameter.
		bin_path = bin_path.parent_path() / "ModixSlicer";
		...
		try {
		    ...
		    // boost::process::spawn() sets SIGCHLD to SIGIGN for the child process, thus if a child ModixSlicer spawns another
		    // subprocess and the subrocess dies, the child ModixSlicer will not receive information on end of subprocess
		    // (posix waitpid() call will always fail).
		    // https://jmmv.dev/2008/10/boostprocess-and-sigchld.html
		    // The child instance of ModixSlicer has to reset SIGCHLD to its default, so that posix waitpid() and similar continue to work.
		    // See GH issue #5507
		}
...
#else // Linux or Unix
	{
		...
		if (args.empty()) {
			...
			my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "modix-slicer" : "modix-gcodeviewer")).string();
