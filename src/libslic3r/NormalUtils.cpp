///|/ Copyright (c) PR 2022 Pavel Mikuš @Godrak
///|/
    ...
    const indexed_triangle_set &its)
{
    ...
    for (const Vec3i &index : its.indices) {
    ...
    const indexed_triangle_set &its)
{
    ...
    for (const Vec3i &indice : its.indices) {
    ...
    const indexed_triangle_set &its)
{
    ...
    for (const Vec3i &indice : its.indices) {
        Vec3f normal = create_triangle_normal(indice, its.vertices);
        Vec3f angles(indice_angle(0, indice.cast<coord_t>(), its.vertices),
                     indice_angle(1, indice.cast<coord_t>(), its.vertices), 0.f);
