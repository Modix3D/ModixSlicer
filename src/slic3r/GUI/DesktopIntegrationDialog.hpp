...
public:
	...
	// returns true if path to ModixSlicer.desktop is stored in App Config and existence of desktop file. 
	...
	// Creates Desktop files and icons for both ModixSlicer and GcodeViewer.
	// Stores paths into App Config.
	// Rewrites if files already existed.
	// if perform_downloader:
    // Creates Destktop files for ModixSlicer downloader feature
	// Regiters ModixSlicer to start on prusaslicer:// URL
	static void perform_desktop_integration();
	// Deletes Desktop files and icons for both ModixSlicer and GcodeViewer at paths stored in App Config.
