...
@ECHO                  console - run and wait on modix-slicer-console.exe
...
@ECHO                  viewer - run modix-gcodeviewer.exe (no wait)
@ECHO                  window - run modix-slicer.exe (no wait)
@ECHO                Default: none
...
@ECHO  Examples:
@ECHO.
@ECHO  Initial build:           build_win -d "c:\src\ModixSlicer-deps"
...
SET START_TIME=%TIME%
SET PS_START_DIR=%CD%
SET PS_SOLUTION_NAME=ModixSlicer
...
msbuild /m ALL_BUILD.vcxproj /p:Configuration=%PS_CONFIG% /v:normal /m:1 %PS_PRIORITY% || GOTO :END
cd ..\..
...
IF "%PS_STEPS%" NEQ "app-cmake" msbuild /m ALL_BUILD.vcxproj /p:Configuration=%PS_CONFIG% /v:normal %PS_PRIORITY% || GOTO :END
...
IF "%PS_RUN%" EQU "console" (
    icacls modix-slicer-console.exe >nul || GOTO :END
    start /wait /b modix-slicer-console.exe
) ELSE IF "%PS_RUN%" EQU "window" (
    icacls modix-slicer.exe >nul || GOTO :END
    start modix-slicer.exe
) ELSE IF "%PS_RUN%" EQU "viewer" (
    icacls modix-gcodeviewer.exe >nul || GOTO :END
    start modix-gcodeviewer.exe
