    ...
    if (!m_gl_initialized) {
...
#elif ENABLE_GL_CORE_PROFILE
                _L("ModixSlicer requires OpenGL %s capable graphics driver to run correctly, \n"
...
#ifdef _WIN32
            message += "\n";
            message += _L("As a workaround, you may run ModixSlicer with a software rendered 3D graphics by running modix-slicer.exe with the --sw-renderer parameter.");
#endif
        	wxMessageBox(message, wxString("ModixSlicer - ") + _L("Unsupported OpenGL version"), wxOK | wxICON_ERROR);
        }
        ...
        if (valid_version) {
            ...
            if (!result) {
                ...
                wxMessageBox(message, wxString("ModixSlicer - ") + _L("Error loading shaders"), wxOK | wxICON_ERROR);
            }
#if ENABLE_GL_CORE_PROFILE
