...
void PreferencesDialog::build()
{
	...
	if (is_editor) {
		...
		append_bool_option(m_optgroup_general, "associate_3mf",
			L("Associate .3mf files to ModixSlicer"),
			L("If enabled, sets ModixSlicer as default application to open .3mf files."),
			app_config->get_bool("associate_3mf"));

		append_bool_option(m_optgroup_general, "associate_stl",
			L("Associate .stl files to ModixSlicer"),
			L("If enabled, sets ModixSlicer as default application to open .stl files."),
...
#if __APPLE__
			L("Allow just a single ModixSlicer instance"),
...
#else
			L("Allow just a single ModixSlicer instance"),
			L("If this is enabled, when starting ModixSlicer and another instance of the same ModixSlicer is already running, that instance will be reactivated instead."),
#endif
		...
		append_bool_option(m_optgroup_general, "default_action_on_close_application",
			...
			L("Always ask for unsaved changes in presets, when: \n"
						"- Closing ModixSlicer while some presets are modified,\n"
		...
		append_bool_option(m_optgroup_general, "associate_gcode",
			L("Associate .gcode files to ModixSlicer G-code Viewer"),
			L("If enabled, sets ModixSlicer G-code Viewer as default application to open .gcode files."),
			app_config->get_bool("associate_gcode"));
		append_bool_option(m_optgroup_general, "associate_bgcode",
			L("Associate .bgcode files to ModixSlicer G-code Viewer"),
			L("If enabled, sets ModixSlicer G-code Viewer as default application to open .bgcode files."),
	...
	append_bool_option(m_optgroup_general, "restore_win_position",
		...
		L("If enabled, ModixSlicer will be open at the position it was closed"),
...
/*
		append_bool_option(m_optgroup_gui, "suppress_hyperlinks",
			...
			L("If enabled, ModixSlicer will not open a hyperlinks in your browser."),
...
*/
	...
	if (is_editor) {
		...
		append_bool_option(m_optgroup_other, "downloader_url_registered",
			...
			L("If enabled, ModixSlicer will be allowed to download from Printables.com"),
